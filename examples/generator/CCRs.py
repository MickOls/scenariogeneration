...
class Scenario(ScenarioGenerator):
    def __init__(self):
        ...
        self.parameters['ego_speedvalue'] = [x for x in range(10,85,5)]
    ...
    def scenario(self,**kwargs):
        ...
        startpos = 25 + kwargs['ego_speedvalue']/3.6* (acceleration_time+ttc_at_speed)
        ...
        ego_action = xosc.AbsoluteSpeedAction(kwargs['ego_speedvalue']/3.6,xosc.TransitionDynamics(xosc.DynamicsShapes.linear,xosc.DynamicsDimension.time,acceleration_time))
        ...
        sce = xosc.Scenario('CCRs v: ' +str(kwargs['ego_speedvalue']) + ', offset: ' + str(kwargs['offset']),'Mandolin',paramdec,entities=entities,storyboard = sb,roadnetwork=road,catalog=catalog)
        return sce

if __name__ == '__main__':
    sce = Scenario()
    sce.print_permutations()
    files = sce.generate('CCRs_scsenarios',None,False)

    # uncomment the following lines to display the scenario using esmini
    from scenariogeneration import esmini
#    esmini(sce,os.path.join('..\..\..\esmini'),'60 60 800 400', False, False,False,'','random')
    esmini(sce,os.path.join('esmini'),'60 60 800 400', False, False,False,'','random')
