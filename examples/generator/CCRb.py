...
class Scenario(ScenarioGenerator):
    ...
    def scenario(self,**kwargs):
        ...
        targetstart = xosc.TeleportAction(xosc.LanePosition(25+kwargs['distance'],0,-1,1))
        ...
        target_slowingdown_action = xosc.AbsoluteSpeedAction(0,xosc.TransitionDynamics(xosc.DynamicsShapes.linear,xosc.DynamicsDimension.rate,abs(kwargs['decel'])))
        ...
        sb = xosc.StoryBoard(init,xosc.ValueTrigger('stop_simulation',2,xosc.ConditionEdge.rising,xosc.SimulationTimeCondition(acceleration_time + np.ceil(np.sqrt(2*kwargs['distance']/abs(kwargs['decel']))) + kwargs['distance']/50 ,xosc.Rule.greaterThan),'stop'))
        sb.add_story(story)

        ## create and return the scenario
        sce = xosc.Scenario('CCRb, distance: ' +str(kwargs['distance']) + ', decelleration: ' + str(kwargs['decel']),'Mandolin',paramdec,entities=entities,storyboard = sb,roadnetwork=road,catalog=catalog)
        return sce

if __name__ == '__main__':
    sce = Scenario()
    #sce.print_permutations()
    files = sce.generate('CCRb_scenarios',None,False)
